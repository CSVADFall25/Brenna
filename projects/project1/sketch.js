...
function setup() {
  pixelDensity(1);
  createCanvas(windowWidth, windowHeight/2); // watercolor canvas area
  background(255);
  ...
  instructionsText = createP(
    'Welcome! Click and drag on the white canvas above to paint with watercolors. '
  );
  instructionsText.position(800, height + 150);
  instructionsText.style('font-size', '18px');
  instructionsText.style('color', 'white');
  instructionsText.style('background', 'rgba(132, 111, 62, 0.6)');
  instructionsText.style('padding', '10px 20px');
  instructionsText.style('border-radius', '15px');
  instructionsText.style('box-shadow', '0 4px 10px rgba(0,0,0,0.1)');
  instructionsText.style('backdrop-filter', 'blur(4px)');
  instructionsText.style('text-align', 'center');
  instructionsText.style('max-width', '400px');
  ...
  colorPicker.position(475, height+90);
  colorPicker.style('border-radius', '8px');
  ...
  paintDropText = createP("Paint Dropper")
  paintDropText.position(70, height+10);
  paintDropText.style('font-size', '16px');

  buttonDry = createButton("Dry All");
  buttonDry.position(210, height + 5);
  buttonDry.style('background-color', 'black');
  buttonDry.style('color', 'white');
  buttonDry.style('border', 'none');
  buttonDry.style('border-radius', '8px');
  buttonDry.style('padding', '4px 8px');

  buttonWet = createButton("Keep Wet");
  buttonWet.position(270, height + 5);
  buttonWet.style('background-color', 'black');
  buttonWet.style('color', 'white');
  buttonWet.style('border', 'none');
  buttonWet.style('border-radius', '8px');
  buttonWet.style('padding', '4px 8px');
  
  buttonDefault = createButton("Default Dry");
  buttonDefault.position(350, height + 5);
  buttonDefault.style('background-color', 'black');
  buttonDefault.style('color', 'white');
  buttonDefault.style('border', 'none');
  buttonDefault.style('border-radius', '8px');
  buttonDefault.style('padding', '4px 8px');
  state = createElement("state", "Default");
  state.position(450, height + 5);

  // Add reset button
  buttonReset = createButton("Clear Canvas");
  buttonReset.position(550, height + 5);
  buttonReset.style('background-color', 'rgba(83, 84, 72, 0.6)');
  buttonReset.style('color', 'white');
  buttonReset.style('border', 'none');
  buttonReset.style('border-radius', '8px');
  buttonReset.style('padding', '4px 8px');

  // Add filter on/off button
  // buttonFilters = createButton("Turn Filters Off/On");
  // buttonFilters.position(210, height + 50);
  // buttonFilters.style('background-color', 'black');
  // buttonFilters.style('color', 'white');
  // buttonFilters.style('border', 'none');
  // buttonFilters.style('border-radius', '8px');
  // buttonFilters.style('padding', '4px 8px');
  // filterState = createElement("filterState", "Filters On");
  ...
  skillRadioButton.position(screenWidth/2+490, height+10);
  skillRadioButton.option('Beginner');
  skillRadioButton.option('Novice');
  skillRadioButton.option('Advanced');
  skillRadioButton.option('No Photo');
  skillRadioButton.selected('No Photo'); // default

  refImageText = createP("Don't know where to start? Select your watercoloring skill level here: ")
  refImageText.position(screenWidth/2+10, height+10);
  refImageText.style('font-size', '16px');

  // Initiailize palette boxes
  paletteBoxes = [];
  paletteBox1 = new PaletteBox(70, height+150, 120, 70);
  paletteBox2 = new PaletteBox(200, height+150, 120, 70);
  paletteBox3 = new PaletteBox(330, height+150, 120, 70);
  paletteBox4 = new PaletteBox(70, height+225, 120, 70);
  paletteBox5 = new PaletteBox(200, height+225, 120, 70);
  paletteBox6 = new PaletteBox(330, height+225, 120, 70);
  paletteBox7 = new PaletteBox(70, height+300, 120, 70);
  paletteBox8 = new PaletteBox(200, height+300, 120, 70);
  paletteBox9 = new PaletteBox(330, height+300, 120, 70);
  paletteBoxes.push(paletteBox1, paletteBox2, paletteBox3, paletteBox4, paletteBox5, paletteBox6, paletteBox7, paletteBox8, paletteBox9);

  addColorButton = createButton("Add Color to Palette");
  addColorButton.position(550, height+90);
  addColorButton.style('background-color', colorPicker.color());
  addColorButton.style('color', 'white');
  addColorButton.style('border', 'none');
  addColorButton.style('border-radius', '8px');
  addColorButton.style('padding', '4px 8px');
...
function draw() {
  ...
  let currSkill = skillRadioButton.value();
  if (currSkill != prevSkill){
    clearCanvas();
    changeRefImage(currSkill);
  }
  prevSkill = currSkill;
...
function changeRefImage(level){
  if (level == "Beginner") {
    ...
    refImg.size(screenWidth/2, height); 
  }
...
function removeFilters(){
  ...
  if (filterStatus == true) {
    filterState.html("Filters On");
  }
  else if (filterStatus == false){
    filterState.html("Filters Off");
  }
}

function saveImage(){
  save("watercolorArtwork.jpg"); // Save as jpg
}
